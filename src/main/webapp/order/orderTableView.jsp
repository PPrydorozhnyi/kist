...
<div class="container">
    <form:form cssClass="col-2 margin-table-view" method="GET" action="${contextPath}/order/create"
               class="form-signin">
        <button class="btn btn btn-success" type="submit">Create new order</button>
    </form:form>
    <hr>
    <div class="row">
        <div class="col-10">
            <table class="table table-bordered table-hover"><caption>Order table</caption>
                ...
                <c:forEach items="${orders}" var="order">
                    <tr>
                        <td><a href="${contextPath}/order/${order.id}">${order.orderNo}</a></td>
                        <td>${order.orderDate}</td>
                        <td>${order.orderText}</td>
                        <td>${order.orderKind.name}</td>
                    </tr>
                </c:forEach>
                </tbody>
            </table>
